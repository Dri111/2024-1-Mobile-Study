...
class ApiState extends State<ApiResultWidget>{
  ...
  Future<Model> callApi() async {
    ...
    return await WeatherService(dio).getWeather(cityName, apikey);
  }
  void updateUI() async{
    Model response = await callApi();
    setState(() {
      temper = (response.main.temp.toDouble()-273.15)*1000;
      humid = response.main.humidity.toDouble()*1000;
      pres=response.main.pressure.toDouble()*0.000987*1000;
      winds=response.wind.speed.toDouble()*1000;
